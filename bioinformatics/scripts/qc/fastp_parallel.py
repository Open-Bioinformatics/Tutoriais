...
# fastp_parallel.py
#
# Processa em paralelo todos os FASTQ pareados (ou single-end) de uma pasta
# com fastp, e gera um relatório HTML agregado (overall.html) com as curvas
# de qualidade/GC e taxas Q20/Q30 antes/depois do trimming de todas as
# amostras.
#
# Pula (skip) amostras cujo output limpo (.clean.fastq.gz) já exista.
#
# Ferramenta: fastp (testado com fastp v1.1.0)
# https://github.com/OpenGene/fastp
#
# Uso (parâmetros usados no mestrado, seguindo Xu et al. 2025,
# https://doi.org/10.1016/j.synbio.2025.07.006):
...
def processDir(folder, options):
    ...
    for opt in options_list:
        ...
        # --- checa se a amostra já foi processada (skip) ---
        already_done = False
        if opt.out_dir:
            if not os.path.exists(opt.out_dir):
                os.makedirs(opt.out_dir)
            out_prefix1 = os.path.join(opt.out_dir, os.path.basename(getBaseName(opt.read1_file)))
            out1 = out_prefix1 + ".clean.fastq.gz"
            cmd += " -o " + out1
            outputs_to_check = [out1]
            if hasattr(opt, 'read2_file'):
                out_prefix2 = os.path.join(opt.out_dir, os.path.basename(getBaseName(opt.read2_file)))
                out2 = out_prefix2 + ".clean.fastq.gz"
                cmd += " -O " + out2
                outputs_to_check.append(out2)
            already_done = all(os.path.exists(o) and os.path.getsize(o) > 0 for o in outputs_to_check)

        if already_done:
            print(f"[SKIP] {opt.read1_file} já processado (output existe em {opt.out_dir})")
            skipped += 1
            continue
    ...
    if skipped:
        print(f"[INFO] {skipped} amostra(s) já processada(s), puladas.")
    ...
    if len(commands) == 0:
        print("Todas as amostras já foram processadas anteriormente. Nada a fazer.")
        return

    if options.parallel is None:
        options.parallel = max(1, os.cpu_count() // 4)

    # NOTA: usar options.parallel (não opt.parallel) -- opt é só a última
    # amostra do loop acima, mas o valor é o mesmo (copiado de options).
